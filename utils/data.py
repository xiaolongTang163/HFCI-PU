...
from utils.pu1k import load_model_net40_h5_data
...
class H5Dataset(data.Dataset):
    ...
    def __init__(self, h5_path, num_patch_point,
        ...
        if is_model_net40:  # BN3 BN3 B
            self.input_array, label_array, self.data_radius = load_model_net40_h5_data(
                data_dir=h5_path, partition=phase, up_ratio=up_ratio, is_debug=is_debug)
            self.label_array = {'x4': label_array}
        else:
            self.input_array, self.label_array, self.data_radius = self.load_h5_data(
                h5_path, up_ratio, num_patch_point)
        print("total of {} batches".format(len(self.input_array)))
    ...
    def load_h5_data(self, h5_path, up_ratio, num_point):
        # print("========== Loading Data ==========")
    ...
    def __getitem__(self, index):
        ...
        if self.is_de:
            return {'pcl_low': input_patches.permute(1, 0).clone(),
                    'pcl_high': label_patches.permute(1, 0).clone(),
                    }
        else:
