...
from utils.model_loss import ChamferLoss
...
class Model(object):
    def __init__(self, net, phase, opt):
        ...
        if self.phase == 'train':
            ...
            self.old_lr = opt.up_lr_init
            self.lr = opt.up_lr_init
    ...
    def forward(self, is_up=True):
        if self.gt is not None:
            self.predicted = self.net(self.input, True)
        else:
